...
const dedup = async (stored, added) => {
	for (let i = 0; i < added.length; i++) {
		...
		let idx = stored.findIndex(
			(storedAuthor) => storedAuthor.email === newAuthor.email
		);
		if (idx > -1) {
...
const add = async (opt) => {
	if (opt.l || opt.s) {
		exec(
			'git --no-pager shortlog master --summary --numbered --email',
			async (err, stdout) => {
				if (err) console.err('Failed to execute git command');
				let usersFromLog = stdout
					.split('\n')
					...
					.sort((a1, a2) => {
						return a1.name.toUpperCase() < a2.name.toUpperCase()
							? -1
							: 1;
					});
				...
				fs.writeFile(
					mommitFile,
					JSON.stringify(await dedup(storedAuthors, authors)),
					{ flag: 'w' }
				);
			}
		);
		...
		const answer = await inquirer.prompt([
			{
				...
				validate: function (answer) {
					return answer.trim() === ''
						? 'You should answer with your friend name =('
						: true;
				},
			},
			{
				...
				validate: function (answer) {
					return answer.trim() === ''
						? 'You should input a valid e-mail'
						: true;
				},
			},
		]);
...
const remove = async () => {
	...
	const newList = authors.filter((author) => {
		return !answers.authors.includes(author.name);
	});
